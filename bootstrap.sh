...
cd /usr/src/ && wget https://mirrors.edge.kernel.org/pub/linux/kernel/v5.x/linux-5.5.tar.xz && tar -xvf linux-5.5.tar.xz
...
reboot
uname -r >> /root/kernel.txt
cd /usr/src/linux-5.5 && make defconfig && make -j5
